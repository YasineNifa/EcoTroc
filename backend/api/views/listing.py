...
class ListingViewSet(viewsets.ModelViewSet):
    ...
    def reserve(self, request, pk=None):
        ...
        accepted_proposition = (
            Proposition.objects.filter(
                listing=listing, buyer=buyer, status=Proposition.Status.ACCEPTED
            )
            .order_by("-created_at")
            .first()
        )
    ...
    def make_offer(self, request, pk=None):
        ...
        if user_profile.jeton_balance < offer_amount:
            return Response(
                {"detail": "Insufficient jeton balance."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if listing.status != Listing.Status.AVAILABLE:
            return Response(
                {"detail": "This listing is not available for offers."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def last_accepted_proposition(self, request, pk=None):
        ...
        if proposition:
            ...
            return Response(
                {"detail": "No accepted proposition found."},
